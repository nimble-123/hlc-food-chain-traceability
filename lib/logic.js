...
async function changeMaterialOwnership(tx) {
    ...
    const actualOwner = tx.material.owner;

    if (material.owner !== getCurrentParticipant()) {
        throw new Error('You are not allowed to manipulate this asset!');
    }

    const materialRegistry = await getAssetRegistry(NS + '.Material');
    // put actual owner to the history stack
    material.ownerHistory.push(actualOwner);
